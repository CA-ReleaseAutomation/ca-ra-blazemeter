 ...
 */
@ActionDescriptor(
        name = "BlazeMeter - Update Test Location",
        description = "List all BlazeMeter harbors (Private Locations) for an account and workspace.",
        category="BlazeMeter." )
    public class BlazeMeterUpdateTestLocation extends Action {
    private static final long serialVersionUID = 1L;
    private static final Logger log = Logger.getLogger(BlazeMeterUpdateTestLocation.class);
    ...
    @ParameterDescriptor(
        ...
        @ParameterDescriptor(
        name = "Location",
        description = "Location of Harbor ID (Harbor-<ID>)",
        out = false,
        in = true,
        nullable = true, // parameter not required
        order = 30,
    ...
    public ActionResult execute() {
        ...
        try {
            log.debug("A call was received to execute a Restful Action: " + "BlazeMeter - Update Test Location");

            String rawRequestValue = "{   \"configuration\": {\n            \"location\": \"${Location}\"}}";

            response = new RestClientManager().restCall(HttpMethod.POST, authType, getInput(),
                "https://a.blazemeter.com/api/v2/tests/${test id}", 70, "", "application/json",
            ...
            response = new HttpRDKResponse("There was an error trying to execute a REST Call Action: '"
                + "BlazeMeter - Update Test Location" + "' for Action Pack: "   + "", null, -1);
    ...
    private List<InputParam> getInput() {
            ...
            inputParameters.add(p);     
                    p = new InputParam();
            p.setName("Location");
                                    p.setVariableName("location");
            p.setPrefix("");
            p.setType(VarType.String);
                                                                     p.setValue(location3);
