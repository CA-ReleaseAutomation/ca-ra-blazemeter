 ...
 */
@ActionDescriptor(
        name = "BlazeMeter - Update Test Engine Count",
        description = "List all BlazeMeter harbors (Private Locations) for an account and workspace.",
        category="BlazeMeter." )
    public class BlazeMeterUpdateTestEngineCount extends Action {
    private static final long serialVersionUID = 1L;
    private static final Logger log = Logger.getLogger(BlazeMeterUpdateTestEngineCount.class);
    ...
    @ParameterDescriptor(
        ...
        @ParameterDescriptor(
        name = "EngineCount",
        description = "EngineCount Description",
        out = false,
        in = true,
        nullable = true, // parameter not required
        order = 30,
    ...
    public void settestid2(String testid2) {
        this.testid2 = testid2;
                ...
                public Integer getenginecount3() {
        return enginecount3;
    } 
    
    public void setenginecount3(Integer enginecount3) {
        this.enginecount3 = enginecount3;
    }
        // Getters and Setters
    ...
    public ActionResult execute() {
        ...
        try {
            log.debug("A call was received to execute a Restful Action: " + "BlazeMeter - Update Test Engine Count");

            String rawRequestValue = "{\"configuration\": {\"serversCount\": ${EngineCount}}}";
            ...
            response = new HttpRDKResponse("There was an error trying to execute a REST Call Action: '"
                + "BlazeMeter - Update Test Engine Count" + "' for Action Pack: "   + "", null, -1);
    ...
    private List<InputParam> getInput() {
            ...
            inputParameters.add(p);     
                    p = new InputParam();
            p.setName("EngineCount");
                                    p.setVariableName("enginecount");
            p.setPrefix("");
            p.setType(VarType.Integer);
                                                                                     if(enginecount3 != null) {
                    p.setValue(enginecount3.toString());
                }
