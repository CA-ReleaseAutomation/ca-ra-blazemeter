 ...
 */
@ActionDescriptor(
        name = "BlazeMeter - Run Test",
        description = "List all BlazeMeter harbors (Private Locations) for an account and workspace.",
        category="BlazeMeter." )
    public class BlazeMeterRunTest extends Action {
    private static final long serialVersionUID = 1L;
    private static final Logger log = Logger.getLogger(BlazeMeterRunTest.class);
    ...
    public ActionResult execute() {
        ...
        try {
            log.debug("A call was received to execute a Restful Action: " + "BlazeMeter - Run Test");

            String rawRequestValue = "";

            response = new RestClientManager().restCall(HttpMethod.POST, authType, getInput(),
                "https://a.blazemeter.com:443/api/v4/tests/${test id}/start", 70, "", "",
            ...
            response = new HttpRDKResponse("There was an error trying to execute a REST Call Action: '"
                + "BlazeMeter - Run Test" + "' for Action Pack: "   + "", null, -1);
            ...
            log.debug("Output is empty");
              ...
              exitcode3 = Integer.valueOf(stringMatch);
                ...
                log.error("Caught exception during populating output parameter: " + "Exit Code", e);
                    ...
                    try {
                Pattern pattern = Pattern.compile("r-v.{15}");
              ...
              sessionid4 = String.valueOf(stringMatch);
            } catch (PatternSyntaxException e) {
                log.error("Caught exception during populating output parameter: " + "SessionID" + ". Bad pattern", e);
            } catch (Exception e) {
                log.error("Caught exception during populating output parameter: " + "SessionID", e);
            }
                }
