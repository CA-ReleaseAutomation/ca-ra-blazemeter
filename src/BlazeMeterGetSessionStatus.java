 ...
 */
@ActionDescriptor(
        name = "BlazeMeter - Get session status",
        description = "List all BlazeMeter harbors (Private Locations) for an account and workspace.",
        category="BlazeMeter." )
    public class BlazeMeterGetSessionStatus extends Action {
    private static final long serialVersionUID = 1L;
    private static final Logger log = Logger.getLogger(BlazeMeterGetSessionStatus.class);
    ...
    public ActionResult execute() {
        ...
        try {
            log.debug("A call was received to execute a Restful Action: " + "BlazeMeter - Get session status");

            String rawRequestValue = "{   \"configuration\": {\n            \"location\": \"${Location}\"}}";

            response = new RestClientManager().restCall(HttpMethod.GET, authType, getInput(),
                "https://a.blazemeter.com:443/api/v4/sessions/${SessionID}", 60, "", "application/json",
            ...
            response = new HttpRDKResponse("There was an error trying to execute a REST Call Action: '"
                + "BlazeMeter - Get session status" + "' for Action Pack: "   + "", null, -1);
            ...
            log.debug("Output is empty");
              ...
              exitcode3 = Integer.valueOf(stringMatch);
                ...
                log.error("Caught exception during populating output parameter: " + "Exit Code", e);
                    ...
                    try {
                                                        	                			List<String> objectMatch = JsonPath.read(output.getContent(), "$..status");
                		                        System.out.println("Object Match2 " + objectMatch);
                                                    
                if (objectMatch != null) {
                                                                                            status4 = new String[objectMatch.size()];
                                                        	status4 = objectMatch.toArray(status4);
                                                                                        }
    ...
    private List<InputParam> getInput() {
            ...
            inputParameters.add(p);     
                    p = new InputParam();
            p.setName("SessionID");
                                    p.setVariableName("sessionid");
            p.setPrefix("");
            p.setType(VarType.String);
                                                                     p.setValue(sessionid2);
    ...
    private List<OutputParam> getOutput() {
            ...
            p.setType(VarType.String);
                ...
                p.setFilterValue( Arrays.toString(status4) );
