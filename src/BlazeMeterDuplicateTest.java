 ...
 */
@ActionDescriptor(
        name = "BlazeMeter - Duplicate Test",
        description = "List all BlazeMeter harbors (Private Locations) for an account and workspace.",
        category="BlazeMeter." )
    public class BlazeMeterDuplicateTest extends Action {
    private static final long serialVersionUID = 1L;
    private static final Logger log = Logger.getLogger(BlazeMeterDuplicateTest.class);
    ...
    public ActionResult execute() {
        ...
        try {
            log.debug("A call was received to execute a Restful Action: " + "BlazeMeter - Duplicate Test");

            String rawRequestValue = "";

            response = new RestClientManager().restCall(HttpMethod.POST, authType, getInput(),
                "https://a.blazemeter.com:443/api/v2/tests/${test id}/duplicate", 30, "", "",
            ...
            response = new HttpRDKResponse("There was an error trying to execute a REST Call Action: '"
                + "BlazeMeter - Duplicate Test" + "' for Action Pack: "   + "", null, -1);
    ...
    private void populateOutputParameters(HttpRDKResponse output) {
        if (StringUtils.isBlank(output.getContent())) {
            log.debug("Output is empty");
              ...
              id4 = String.valueOf(stringMatch);
            } catch (PatternSyntaxException e) {
                log.error("Caught exception during populating output parameter: " + "id" + ". Bad pattern", e);
            } catch (Exception e) {
                log.error("Caught exception during populating output parameter: " + "id", e);
            }
                }
    ...
    private List<OutputParam> getOutput() {
            ...
            outputParameters.add(p);                    
                    p = new OutputParam();
            p.setName("id");
                                    p.setVariableName("id");
            p.setType(VarType.String);
            
                            p.setFilterType(FilterType.REGEX);
                        
                                                                     p.setFilterValue( id4 );
