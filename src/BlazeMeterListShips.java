 ...
 */
@ActionDescriptor(
        name = "BlazeMeter - List Ships",
        description = "List all BlazeMeter harbors (Private Locations) for an account and workspace.",
        category="BlazeMeter." )
    public class BlazeMeterListShips extends Action {
    private static final long serialVersionUID = 1L;
    private static final Logger log = Logger.getLogger(BlazeMeterListShips.class);
    ...
    @ParameterDescriptor(
        ...
        @ParameterDescriptor(
        name = "harbor Id",
        description = "harbor Id Description",
        out = false,
        in = true,
        nullable = true, // parameter not required
        order = 20,
    ...
    public void setapikey1(Password apikey1) {
        this.apikey1 = apikey1;
                ...
                public String getharborid2() {
        return harborid2;
    } 
    
    public void setharborid2(String harborid2) {
        this.harborid2 = harborid2;
    }
        // Getters and Setters
    ...
    public void setexitcode3(Integer exitcode3) {
        this.exitcode3 = exitcode3;
                            ...
                            public String[] getshipids4() {
                return shipids4;
            }
            
            public void setshipids4(String[] shipids4) {
                this.shipids4 = shipids4;
            }
                                    public String[] getshipnames5() {
                return shipnames5;
            }
            
            public void setshipnames5(String[] shipnames5) {
                this.shipnames5 = shipnames5;
            }
                                    public String[] getshipaddresses6() {
                return shipaddresses6;
            }
            
            public void setshipaddresses6(String[] shipaddresses6) {
                this.shipaddresses6 = shipaddresses6;
            }
    ...
    public ActionResult execute() {
        ...
        try {
            log.debug("A call was received to execute a Restful Action: " + "BlazeMeter - List Ships");

            String rawRequestValue = "";

            response = new RestClientManager().restCall(HttpMethod.GET, authType, getInput(),
                "https://a.blazemeter.com:443/api/v4/harbors/${harbor Id}/ships", 30, "", "",
            ...
            response = new HttpRDKResponse("There was an error trying to execute a REST Call Action: '"
                + "BlazeMeter - List Ships" + "' for Action Pack: "   + "", null, -1);
            ...
            log.debug("Output is empty");
              ...
              exitcode3 = Integer.valueOf(stringMatch);
                ...
                log.error("Caught exception during populating output parameter: " + "Exit Code", e);
                    ...
                    try {
                                                        	                			List<String> objectMatch = JsonPath.read(output.getContent(), "$.result..id");
                		                        System.out.println("Object Match2 " + objectMatch);
                                                    
                if (objectMatch != null) {
                                                                                            shipids4 = new String[objectMatch.size()];
                                                        	shipids4 = objectMatch.toArray(shipids4);
                                                                                        }
                ...
                log.error("Caught exception during JsonPath matching", e);
                    ...
                    try {
                                                        	                			List<String> objectMatch = JsonPath.read(output.getContent(), "$.result..name");
                		                        System.out.println("Object Match2 " + objectMatch);
                                                    
                if (objectMatch != null) {
                                                                                            shipnames5 = new String[objectMatch.size()];
                                                        	shipnames5 = objectMatch.toArray(shipnames5);
                                                                                        }
                ...
                log.error("Caught exception during JsonPath matching", e);
                    ...
                    try {
                                                        	                			List<String> objectMatch = JsonPath.read(output.getContent(), "$.result..address");
                		                        System.out.println("Object Match2 " + objectMatch);
                                                    
                if (objectMatch != null) {
                                                                                            shipaddresses6 = new String[objectMatch.size()];
                                                        	shipaddresses6 = objectMatch.toArray(shipaddresses6);
                                                                                        }
                ...
                log.error("Caught exception during JsonPath matching", e);
                    ...
                    try {
                                                        	                			List<String> objectMatch = JsonPath.read(output.getContent(), "$.result..state");
                		                        System.out.println("Object Match2 " + objectMatch);
                                                    
                if (objectMatch != null) {
                                                                                            status7 = new String[objectMatch.size()];
                                                        	status7 = objectMatch.toArray(status7);
                                                                                        }
    ...
    private List<InputParam> getInput() {
            ...
            inputParameters.add(p);     
                    p = new InputParam();
            p.setName("harbor Id");
                                    p.setVariableName("harborid");
            p.setPrefix("");
            p.setType(VarType.String);
                                                                     p.setValue(harborid2);
    ...
    private List<OutputParam> getOutput() {
            ...
            outputParameters.add(p);                    
                    p = new OutputParam();
            p.setName("Ship IDs");
                                    p.setVariableName("shipids");
            p.setType(VarType.String);
            
                            p.setFilterType(FilterType.JSON);
                        
                                        
                p.setFilterValue( Arrays.toString(shipids4) );
                        
            outputParameters.add(p);                    
                    p = new OutputParam();
            p.setName("Ship Names");
                                    p.setVariableName("shipnames");
            p.setType(VarType.String);
            
                            p.setFilterType(FilterType.JSON);
                        
                                        
                p.setFilterValue( Arrays.toString(shipnames5) );
                        
            outputParameters.add(p);                    
                    p = new OutputParam();
            p.setName("Ship Addresses");
                                    p.setVariableName("shipaddresses");
            p.setType(VarType.String);
            
                            p.setFilterType(FilterType.JSON);
                        
                                        
                p.setFilterValue( Arrays.toString(shipaddresses6) );
                        
            outputParameters.add(p);                    
                    p = new OutputParam();
            p.setName("status");
                                    p.setVariableName("status");
            p.setType(VarType.String);
            
                            p.setFilterType(FilterType.JSON);
                        
                                        
                p.setFilterValue( Arrays.toString(status7) );
