 ...
 */
@ActionDescriptor(
        name = "BlazeMeter - List Harbors",
        description = "List all BlazeMeter harbors (Private Locations) for an account and workspace.",
        category="BlazeMeter." )
    public class BlazeMeterListHarbors extends Action {
    private static final long serialVersionUID = 1L;
    private static final Logger log = Logger.getLogger(BlazeMeterListHarbors.class);
    ...
    public void setapikey1(Password apikey1) {
        this.apikey1 = apikey1;
                ...
                public String getworkspaceid2() {
        return workspaceid2;
    } 
    
    public void setworkspaceid2(String workspaceid2) {
        this.workspaceid2 = workspaceid2;
    }
    ...
    public ActionResult execute() {
        ...
        try {
            log.debug("A call was received to execute a Restful Action: " + "BlazeMeter - List Harbors");

            String rawRequestValue = "";

            response = new RestClientManager().restCall(HttpMethod.GET, authType, getInput(),
                "https://a.blazemeter.com:443/api/v4/harbors?workspaceId=${workspaceId}&accountId=${accountId}&limit=10", 60, "", "",
            ...
            response = new HttpRDKResponse("There was an error trying to execute a REST Call Action: '"
                + "BlazeMeter - List Harbors" + "' for Action Pack: "   + "", null, -1);
    ...
    private List<InputParam> getInput() {
            ...
            inputParameters.add(p);     
                    p = new InputParam();
            p.setName("workspaceId");
                                    p.setVariableName("workspaceid");
            p.setPrefix("");
            p.setType(VarType.String);
                                                                     p.setValue(workspaceid2);
                                             
            inputParameters.add(p);     
                    p = new InputParam();
            p.setName("accountId");
                                    p.setVariableName("accountid");
            p.setPrefix("");
            p.setType(VarType.String);
                                                                     p.setValue(accountid3);
